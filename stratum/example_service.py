...
class ExampleService(GenericService):
    ...
    synchronous.params = [
        ('how_long', 'int', 'For how many seconds the algorithm should run.'),
    ]
...
class PubsubExampleService(GenericService):
    ...
    subscribe.params = [
        ('period', 'int',
         'Broadcast to the client only if timestamp%period==0. Use 1 for receiving an event in every second.'),
    ]
    ...
    unsubscribe.params = [
        ('subscription_key', 'string', 'Key obtained by calling of subscribe method.'),
    ]
